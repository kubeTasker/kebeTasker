...
import (
	...
	"github.com/kubeTasker/pkg/file"
	taskers3 "github.com/kubeTasker/pkg/s3"
...
func (s3Driver *S3ArtifactDriver) newS3Client() (taskers3.S3Client, error) {
	opts := taskers3.S3ClientOpts{
		...
		Trace:     os.Getenv(common.EnvVarTaskerTrace) == "1",
	}
	return taskers3.NewS3Client(opts)
}
...
func (s3Driver *S3ArtifactDriver) Load(inputArtifact *wfv1.Artifact, path string) error {
	err := wait.ExponentialBackoff(wait.Backoff{Duration: time.Second * 2, Factor: 2.0, Steps: 5, Jitter: 0.1},
		func() (bool, error) {
			...
			if !taskers3.IsS3ErrCode(origErr, "NoSuchKey") {
